<<<<<<< Updated upstream
<<<<<<< Updated upstream
// StudentGrade2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
// Function to calculate the GPA based on letter grades
float calculateGPA(const std::string& letterGrade, const std::vector<GradingScale>& gradingScales) {
    for (const GradingScale& scale : gradingScales) {
        if (letterGrade == scale.getLetterGrade()) {
            return scale.getGradePoint();
        }
    }
    return 0.0; // Return 0.0 if the grade is not found
}

// Function to find subject credit based on the subject name
float findSubjectCredit(const std::string& subjectName, const std::vector<Subject>& subjects) 
{
    ...
    return 0.0; // Return 0.0 if the subject is not found
}

std::string findLetterGrade(int mark, const std::vector<GradingScale>& gradingScales) {
    for (const GradingScale& scale : gradingScales) {
        if (mark >= scale.getLowerBound() && mark <= scale.getUpperBound()) {
...
int main()
{
    ...
    // Create an output file called "OOPstudent_grades.txt" to write the results.
    ...
    OutputFile << "\tWeighted_Credit\tEarned_Credit_Hours\tGPA" << std::endl;

    // Initialize the total weighted credit, earned credit hours, and total credit hours to 0 for each student
    // float weightedCredit = 0.0;
    // float earnedCreditHours = 0.0;
    // float totalCreditHours = 0.0; // not used
    ...
    // Loop through each student
        for (size_t i = 0; i < students.size(); ++i) 
        {
           ...
           // float totalGPA = 0.0; // Initialize the total GPA to 0 for each student
           
            // Initialize the total weighted credit and earned credit hours to 0 for each student
            float weightedCredit = 0.0;
            float earnedCreditHours = 0.0;
            float subjectCredit = 0.0;


                        // Loop through each subject for the current student
                 //        for (const std::string& subjectName : subjectNames) {
                //
                //            int mark = studentMarks[i][subjectName]; // Get the mark for the subject
                //            float subjectCredit = findSubjectCredit(subjectName, subjects); // Get the subject's credit
                //
                //
            for (const std::string& subjectName : subjectNames) 
            {
                ...
                //// Create a non-constant variable to store the trimmed subject name
                // std::string trimmedSubjectName = subjectName;
                //// Trim leading and trailing spaces from the subject name
                //trimmedSubjectName = trimmedSubjectName.substr(trimmedSubjectName.find_first_not_of(' '), trimmedSubjectName.find_last_not_of(' '));
                //int mark = studentMarks[i][trimmedSubjectName];
                */

              int mark = studentMarks[i][subjectName];

                if (mark >= 40) 
                { // Check if the student has passed the subject
                    subjectCredit = findSubjectCredit(subjectName, subjects);
                    earnedCreditHours += subjectCredit;
                }

                // Debug: Print subject information
                std::cout << "Student: " << students[i].getName() << ", Subject: " << subjectName << ", Mark: " << mark << ", Credit: " << subjectCredit << std::endl;


                // Calculate subject GPA based on the mark and grading scales
                std::string LetterGrade = findLetterGrade(mark, gradingScales);
                float subjectGPA = calculateGPA(findLetterGrade(mark, gradingScales), gradingScales);

                // Calculate the weighted credit and earned credit hours for this subject
                weightedCredit += subjectCredit * subjectGPA;
                //earnedCreditHours += subjectCredit;

                OutputFile << "\t" << LetterGrade; // Write the subject mark to the output file
            }

            // Calculate the student's GPA using the weighted credit and earned credit hours
            float studentGPA = (earnedCreditHours != 0.0) ? (weightedCredit / earnedCreditHours) : 0.0;
            //Ternary operator that checks if earnedCreditHours is not equal to zero.
            // If it is not equal to zero, it divides weightedCredit by earnedCreditHours and assigns the result to studentGPA.
            // If it is equal to zero, it assigns zero to studentGPA

            // Write the calculated values to the output file
            OutputFile << "\t" << weightedCredit << "\t" << earnedCreditHours << "\t" << studentGPA << std::endl;
