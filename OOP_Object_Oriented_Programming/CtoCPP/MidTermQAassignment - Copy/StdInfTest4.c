...
struct  SubjectMarks studentMarks[MAX_STUDENTS];
    ...
    sscanf(Lline, "%*s %*s %[^\t] %[^\t] %[^\t] %[^\t] %[^\t]",studentMarks.subjectNames[i]);
...
for (int i = 0; i < MAX_SUBJECTS; i++) {
    size_t len = strlen(studentMarks.subjectNames[i]);
    if (len > 0 && studentMarks.subjectNames[i][len - 1] == '\n') {
        studentMarks.subjectNames[i][len - 1] = '\0';
    }
}
    ...
    /// Print header with for loop
              printf("Sl\tRoll\t");
            for (int i = 0; i < MAX_SUBJECTS; i++)
            {    printf("%s    ", studentMarks.subjectNames[i]);
   ...
   printf("\n Print extracted subject names and their lengths\t\n");
for (int i = 0; i < 5; i++) {
    printf("Subject %d: \"%s\" (Length: %zu)\n", i + 1, studentMarks.subjectNames[i], strlen(subjectNames[i]));
}
...
/**************************************************************/
    ...
    float CI =0;
    float StCi =0;
    int credit = 3; // Assign 3.0 credit hours to each subject
    for (int i = 0; i < MAX_SUBJECTS; i++) {
           printf("%s\t%.2f\n", subjectNames[i], credit);
