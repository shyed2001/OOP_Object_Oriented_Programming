...
<<<<<<< Updated upstream
<<<<<<< Updated upstream
// Function to calculate CGPA and update earned credits for a student
//float calculateAndWriteCGPA(std::ostream& out, const std::map<std::string, int>& studentMarks, const std::vector<Subject>& subjects, const std::vector<GradingScale>& gradingScales, const std::vector<std::string>& subjectNames) {
//    float totalGradePoints = 0.0;
//    float totalCredits = 0.0;
//    float totalofferedCredits = 0.0;
//    for (const std::string& subjectName : subjectNames) {
//        int mark = studentMarks.at(subjectName);
//       // calculateAndWriteLetterGrade(out, mark, gradingScales); // Calculate and write the letter grade
//
//        // Find the subject's credit hours
//        float subjectCredit = 0.0;
//        for (const Subject& subject : subjects) {
//            if (subject.getName() == subjectName) {
//                subjectCredit = subject.getCredit();
//                //totalofferedCredits += subjectCredit;
//                break;
//            }
//        }
//
...
//        for (const GradingScale& scale : gradingScales) {
//            if (mark >= scale.getLowerBound() && mark <= scale.getUpperBound()) {
//                gradePoint = scale.getGradePoint();
//                break;
//            }
//        }
//
//        // Check if the grade is not 'F' before counting it
//        if (gradePoint > 0.0) {
...
//    }
//
//    float cgpa = (totalCredits != 0.0) ? (totalGradePoints / totalCredits) : 0.0;
//    out << "\t" << std::fixed << std::setprecision(2) << cgpa;
//    //out << "\t" << std::fixed << std::setprecision(2) << totalofferedCredits;
//    out << "\t" << std::fixed << std::setprecision(2) << totalGradePoints;
//    out << "\t" << std::fixed << std::setprecision(2) << totalCredits;
//    return cgpa;
//}
...
float calculateAndWriteCGPA(std::ostream& out, const std::map<std::string, int>& studentMarks, const std::vector<Subject>& subjects, const std::vector<GradingScale>& gradingScales, const std::vector<std::string>& subjectNames) {
    ...
    for (const std::string& subjectName : subjectNames) {
        ...
        for (const Subject& subject : subjects) {
            if (subject.getName() == subjectName) {
                subjectCredit = subject.getCredit();
                totalofferedCredits += subjectCredit; // Increment totalofferedCredits with the subject's credit.
                break;
            }
        }
    ...
    out << "\t" << std::fixed << std::setprecision(2) << cgpa;
    out << "\t" << std::fixed << std::setprecision(2) << totalofferedCredits; // Output totalofferedCredits.
    out << "\t" << std::fixed << std::setprecision(2) << totalGradePoints;
    out << "\t" << std::fixed << std::setprecision(2) << totalCredits;
    return cgpa;
}
...
//This data structure is useful for storing and organizing data where you need to keep track of student marks and associate them with individual student names.
   ...
   // Nothing was done with the first two column values
    ...
    // create an output file called "OOP_S_G.txt" to write our results.
...
<<<<<<< Updated upstream
        ...
        float studentCGPA = calculateAndWriteCGPA(OutputFile2, studentMarks[i], subjects, gradingScales, subjectNames);

        //OutputFile2<<studentCGPA << std::endl;
